...
class NumpyLinearModel():
    ...
    def grad_v(self, X:np.ndarray, v_trgt:np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        ########################
        B = X.shape[0]
        v_pred = self.forward_v(X)  # shape: (B, 1)
        error = v_trgt - v_pred  # shape: (B, 1)
        tanh_deriv = 1 - v_pred ** 2  # shape: (B, 1)
        grad_w_v = -2 * X.T @ (error * tanh_deriv) / B  # shape: (O, 1)
        grad_b_v = -2 * np.mean(error * tanh_deriv, axis=0)  # shape: (1,)
